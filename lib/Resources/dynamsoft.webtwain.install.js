...
function _show_install_dialog(ProductName, objInstallerUrl, bHTML5, iPlatform, bIE, bSafari, bSSL, strIEVersion){
	...
	if(bHTML5) {
		title = 'Скачайте файл <br /> для работы приложения';
	} else {
		if(bIE)
			title = 'Скачайте файл <br /> для работы приложения';
		else
	...
	ObjString.push('<div style="color:#AAAAAA">* Установите файл вручную</div></div>');
	...
	if(bHTML5){
		...
		if(bIE){
			...
			{
			    ObjString.push('<div class="dynamsoft-dwt-dlg-red">После установки перезапустите браузер.</div>');
            } 
            else
			{
				ObjString.push('<div class="dynamsoft-dwt-dlg-red">После установки обновите страницу.</div>');
			}
			ObjString.push('</div>');
		}
		...
		ObjString.push('<div class="dynamsoft-dwt-dlg-tail">');
		if(bIE){
			ObjString.push('После установки, пожалуйста:<br />');
			ObjString.push('1. Обновите страницу<br />');
			ObjString.push('2. Разрешите дополнению "DynamicWebTWAIN" работать.');
		} else {
			ObjString.push('<div class="dynamsoft-dwt-dlg-red">После установки обновите страницу.</div>');
		}
		ObjString.push('</div>');
